...
template <int BLOCK_SIZE> __global__ void test_GPU(int* id)
{
	int bx = blockIdx.x;
	int by = blockIdx.y;
	int tx = threadIdx.x;
	...
	int Nx = blockDim.x;
	int Ny = blockDim.y;
	int idx = Nx * BLOCK_SIZE * BLOCK_SIZE * by + BLOCK_SIZE * BLOCK_SIZE * bx + BLOCK_SIZE * ty + tx;
	id[idx] = idx;
}
...
// s0
__device__ void StokesS0(float* mo, float* v1, float* v1p, float* v2, float* v2p, double iter) {
...
// s1
__device__ void StokesS1(float* mo, float* v1, float* v1p, float* v2, float* v2p, double iter) {
	mo[0] = (mo[0] * iter + v1p[0] * v1[0] - v1p[1] * v1[1] - v2p[0] * v2[0] + v2p[1] * v2[1]) / (iter + 1.);
	mo[1] = (mo[1] * iter + v1p[0] * v1[1] + v1p[1] * v1[0] - v2p[0] * v2[1] - v2p[1] * v2[0]) / (iter + 1.);
}

// Linear Horizontal Pol.
__device__ void StokesLH(float* mo, float* v1, float* v1p, float* v2, float* v2p, double iter) {
	mo[0] = (mo[0] * iter + v1p[0] * v1[0] - v1p[1] * v1[1]) / (iter + 1.);
	mo[1] = (mo[1] * iter + v1p[0] * v1[1] + v1p[1] * v1[0]) / (iter + 1.);
}

// Linear Vertical Pol.
__device__ void StokesLV(float* mo, float* v1, float* v1p, float* v2, float* v2p, double iter) {
